...
We demonstrate the power of this approach on the Gaia TGAS sample of stars with APASS magnitudes.
...
3D distribution of stars in the milky way. 

Parallaxes are fundamental observables for the determination of stellar distances.
However, they are generally difficult to estimate, and restricted to relatively bright stars. 
In particular, the distance error obtained with parallaxes, under a Gaussian approximation, grows as parallax error over parallax squared.
...
In particular, it is well known that stars fall in narrow regions of the color--magnitude space.
Thus, photometric information (\ie apparent magnitudes) can complement parallax information and prove crucial to determine the distance to stars.
Yet, theoretical models predict narrow tracks in color--magnitude, and fail to account for all the diversity of real objects.
...
Our approach relies a standard property of hierarchical probabilistic models: a large number of noisy observations (here, noisy parallaxes and magnitudes) can be used to construct an estimate of the underlying distribution (here, a color--magnitude diagram), which in turn shrinks the uncertainties on their individual properties (here, the distances).
In other words, \textit{the whole is greater than the sum of the parts}: stellar distance estimates can be significantly improved by simultaneously analysing all the available data.
This effect is particularly relevant since Gaia will soon provide hundreds of millions of sources with parallax, proper motion, and multicolor information.
This offers the opportunity to construct accurate models of the distribution of magnitudes, colors, dust, and stellar densities in the Galaxy, which will in turn significantly improve our estimates of the intrinsic properties of individual stars.
The core goal of this paper is to demonstrate how one aspect of this hierarchical inference can be done in practice. 
In particular, we focus on evaluating the shrinkage of distance estimates when constructing a flexible color--magnitude diagram from simultaneously analyzing millions of objects with noisy parallaxes and apparent magnitudes.

We make a number of simplifying assumptions that make our demonstration more transparent, such as adopting uninformative distance priors and point estimates of dust corrections. 
